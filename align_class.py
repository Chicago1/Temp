 ...
 (default = 1)
    ...
    # Partial-pixel shift
        ...
        if usfac > 2:
            ...
            dftshift = npy.fix(npy.ceil(usfac * 1.5) / 2)
...
def subpixel_align(ref, tmp, threshold_flag, threshold, subpixel_flag):
    ...
    if npy.size(ndims) == 2:
        ...
        result = dftregistration(ref_fft[:,:], tmp_fft[:,:], usfac=1000)
        e, p, cl, r, array_shift = result
        x_shift = cl
        y_shift = r
...
def remove_phase_ramp(tmp, threshold_flag, threshold, subpixel_flag):
    tmp_tmp, x_shift, y_shift = subpixel_align(
        sf.ifftshift(
            sf.ifftn(
                sf.fftshift(
                    npy.abs(tmp)))), sf.ifftshift(
            sf.ifftn(
                sf.fftshift(tmp))), threshold_flag, threshold, subpixel_flag)
